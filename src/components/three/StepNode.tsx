...
export function StepNode({ position, label, isActive, isCompleted, isProcessing, onClick, color }: StepNodeProps) {
  ...
  useFrame((state) => {
    ...
    // Processing: gentle spin, otherwise slow sway
    if (isProcessing) {
  ...
  return (
    <group ref={groupRef} position={position}>
      ...
      {/* Border outline */}
      <lineSegments ref={borderRef} rotation={[Math.PI / 2, 0, 0]} position={[0, 0, -0.05]} geometry={borderGeo}>
        <lineBasicMaterial color={currentColor} transparent opacity={0.3} />
      </lineSegments>

      {/* Soft point light for active / completed */}
      {(isActive || isCompleted) && (
        <pointLight color={currentColor} intensity={isActive ? 1.5 : 0.4} distance={2.5} decay={2} />
      )}

      {/* Text label in front of the node */}
      <sprite position={[0, -0.65, 0.5]} scale={[1.8, 0.45, 1]}>
        <spriteMaterial map={labelTexture} transparent depthWrite={false} opacity={isActive ? 1 : 0.85} />
      </sprite>
    </group>
  );
}
